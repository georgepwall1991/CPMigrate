...
internal static class JsonOutputWriter
{
    ...
    /// <param name="announceFile">When <c>true</c>, writes the "JSON output written to: …" notice after a file write (ignored under <c>--quiet</c> or for stdout output). Defaults to <c>true</c>.</param>
    public static async Task EmitAsync(string json, Options options, IConsoleService? consoleService, bool announceFile = true)
    {
