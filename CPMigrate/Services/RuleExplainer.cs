...
public static class RuleExplainer
{
    ...
    private static string? GetExample(AnalysisIssueCode code) => code switch
    {
        ...
        AnalysisIssueCode.RedundantDirectReference => "cpmigrate --analyze --transitive -s ./MySolution.sln",
    ...
    private static string? GetFixHint(AnalysisIssueCode code) => code switch
    {
        ...
        AnalysisIssueCode.RedundantDirectReference => "Remove the direct PackageReference — the package is already provided transitively by another top-level package, so the direct entry only adds maintenance surface.",
