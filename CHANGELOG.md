...
### Added
- **`--diff-file <PATH>`: every dry-run diff can now travel with the CI job.** A `--dry-run --diff` preview renders as colored text on the terminal and is gone the moment the process exits, so a pipeline that wanted to attach the proposed changes to a pull request had to scrape scrollback it does not have. Passing `--diff-file diffs.patch` appends every unified diff generated during the run to that file — one diff per change, separated by a blank line, in the same `--- a/…`/`+++ b/…` format the console shows. The file is created (or truncated) at the start of the run, so an empty artifact means "no changes" while a missing one means the run crashed; collection works with or without `--diff` rendering on screen, and terminal output is unchanged when the flag is absent. Combining `--diff-file` with `--output Json` or `--output Sarif` is rejected up front: machine-readable modes silence the renderer their diffs would come from, and neither payload schema has a place for them.
