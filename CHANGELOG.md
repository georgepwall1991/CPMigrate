...
- **Analysis is about four times faster on a large solution — measured 39s → 9s on 60 projects.** Resolving packages shells out to `dotnet package list` once per project, a separate process that spends most of its half-second waiting on I/O, and the whole reference pass was serial because *part* of it has to be. That pass is now split along the line that actually matters for correctness:
  ...
  - **Every candidate location is locked, not the most likely one.** Which intermediate path is in force depends on MSBuild conditions, imported `Directory.Build.props`, and properties this phase cannot evaluate — evaluating them is precisely what it must not do. So picking one candidate means guessing, and a wrong guess *splits* a lock that should have been shared: two projects that do write to the same file get separate locks and race. The project's own directory and any intermediate path its file declares are all locked, in a fixed order so overlapping candidates cannot deadlock. The cost of being wrong is now two projects serialised unnecessarily, which is a strictly better error than silently fewer findings. **Arrived at over five rounds of cross-review**, each finding a case the previous heuristic got wrong. The keys are also made comparable, which took its own two rounds: the default candidate is the `obj` directory rather than the project directory (so a project redirected into `OtherProject/obj` matches the project that owns it), and a path built from MSBuild properties keeps its literal text as a shared key rather than being discarded (two projects declaring `$(RepoRoot)artifacts/obj/` mean the same place). Nothing in the lookup throws, including path normalisation on a declared value containing an invalid character. And the `Directory.Build.props` files above a project are read for the same two properties, because a repo-wide `artifacts/obj` is normally set once there rather than in every project — the case that was originally documented as a known limit and is now closed. **Also found in cross-review.**
  - The lookup never throws. It reads the project file *before* the project is scanned, so an unreadable or unauthorised project would have aborted the entire analysis — where the scanners are equipped to report that one as an incomplete scan and carry on. A regression this change introduced, **found in cross-review**.
