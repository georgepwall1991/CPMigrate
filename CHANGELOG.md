...
### Changed
- **Analysis is two to four times faster on a large solution.** Measured on 60 projects: 39s → 9.6s on an idle machine, 46s → 19s with the machine loaded. Absolute figures move with load, so the ratio measured back-to-back is the honest number — quoting only the flattering pair would have been comparing an idle parallel run against a busy serial one, which I did once by accident while measuring this. Resolving packages shells out to `dotnet package list` once per project, a separate process that spends most of its half-second waiting on I/O, and the whole reference pass was serial because *part* of it has to be. That pass is now split along the line that actually matters for correctness:
  ...
  - **The lock asks whether the assets file *could* have moved, not where it went.** Six rounds of cross-review each found another route to a shared assets file: a conditional property, one set in an imported `Directory.Build.props`, one built from `$(…)`, `MSBuildProjectExtensionsPath` outranking `BaseIntermediateOutputPath`, `ProjectAssetsFile` naming the file outright, an import reaching a *child* directory. Every fix was correct and the next round found another — which is the signal that the approach was wrong rather than unfinished. Knowing where the file really goes means evaluating the project, and evaluating the project is the one thing this phase must not do.
    - So the question asked is the one that *is* answerable without MSBuild: do `BaseIntermediateOutputPath`, `MSBuildProjectExtensionsPath`, or `ProjectAssetsFile` appear anywhere in the project or the props/targets files reachable from it, imports followed transitively (parsed, not pattern-matched — a regex over the raw text missed `<Import Project='…' />`, and single quotes are just as valid)? If not, `obj/` under the project is where the file goes — provably, since those are the only ways to move it — and the directory key is sound. If any of them appears, or anything cannot be read or resolved, every project shares one lock and the phase runs serially.
    - The ordinary layout keeps the full concurrency; anything unusual gets the previous serial behaviour and no possibility of a race. The check itself is memoised per directory and bounded to MSBuild's own discovery rule — the nearest `Directory.Build.props` and `Directory.Build.targets`, then stop — because an earlier version enumerated every props and targets file in every ancestor up to the filesystem root and cost more than the concurrency it was guarding. A missed redirect silently erases findings; an unnecessary serialisation costs seconds.
...
### Added
- `ScanWorkTests` pins the properties the concurrency depends on rather than a duration, and asserts the concurrency it claims to test actually happened. The gate is sized once per process and ignores later requests for a different limit, so the first draft's parallelism-8 run silently inherited the serial run's single permit — the test would have stayed green while never running anything concurrently. **Found in cross-review**, and it is the same green-for-the-wrong-reason this suite exists to prevent. What it now checks: findings are identical at every parallelism, a solution with projects in one shared directory still reports its inconsistency, and the same solution produces the same report twice (so merging in completion order rather than project order would fail). A timing assertion in CI is a flake generator and would not have caught any of the failures above, all of which produce a *clean* report with a successful exit code.
