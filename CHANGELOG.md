...
### Changed
- **`--update-packages` now asks the same feeds restore uses.** Candidate discovery used to HTTP-GET `api.nuget.org/v3-flatcontainer` for every pin in `Directory.Packages.props`. A package that lived only on Azure Artifacts, GitHub Packages, Artifactory, or an air-gapped mirror came back 404, which the lookup treated as "does not exist" and did **not** record as a failed check — so an org whose internal packages were all behind heard `Everything up to date!`. Worse, if anyone published `Contoso.Core 99.0.0` on nuget.org, CPMigrate proposed that upgrade, wrote it into the props file, and `--bisect` would keep it if tests stayed green. Latest version is now `dotnet list package --outdated` per project, scheduled through `GroupedScanScheduler`, so NuGet.Config sources, credential providers, and package source mapping decide what "latest" means. A package present in the props file but absent from every outdated row is simply not a candidate — that is the correct answer, and it is the dependency-confusion guard. `NuGetVersionLookupService` / `INuGetVersionLookupService` / `NuGetRetryPolicy` are gone; self-update (`cpmigrate --update`) still talks to nuget.org, which is where CPMigrate itself is published. Pinned by `RestoreBackedUpdateCandidateSourceTests` (highest-wins merge, props version over resolved, private-feed pin never proposed, one failed project named in `UnscannedProjects`) and `UpdatePackagesAsync_PackageAbsentFromOutdatedScan_IsNotProposed`.

- **An incomplete `--update-packages` scan is now exit `8`, and it writes nothing.** A project whose outdated query failed used to be skipped; the run could still rewrite the props file and report success for the packages it did see. That is the silent-incomplete verdict every other scan refuses. One unscanned project now prints the project names, carries the guidance in `Warnings` (so `--output Json` / `--quiet` still see it), skips backup creation, and leaves the props file untouched — including under `--dry-run`. A project that cannot restore against the configured feeds would fail the verification restore anyway; failing closed avoids the backup churn. Pinned by `UpdatePackagesAsync_UnscannedProject_ExitsIncompleteAndWritesNothing` and the dry-run / JSON siblings.

- **`--doctor` lists configured NuGet sources instead of claiming "nuget.org reachable".** `dotnet nuget list source --format Detailed` (from the workspace, so a repo-level NuGet.Config is visible) is parsed into named sources; each enabled HTTP source is probed. 200 is reachable, 401/403 is reachable-but-authenticated, anything else names the source. Local folder sources are reported as local and not probed. When the listing itself fails, the old nuget.org service-index probe remains as a fallback. Pinned by `ParseNugetListSource_*` and `SummarizeNugetSources_*`.
