...
### Changed
- **Analysis is several times faster on a large solution — measured 69s → 13s on 30 projects, and 200s → 50s on 60, with byte-identical findings and no scan failures either way.** Resolving packages shells out to `dotnet package list` once per project, a separate process that spends most of its half-second waiting on I/O, and that pass was serial.
  ...
  - One guard remains, and it is a much narrower question than 3.24.0's. The redirection is passed as environment variables, because `dotnet package list` rejects `-p:` arguments — and an assignment in the project or an imported `Directory.Build.props` overrides them. Measured: a props file setting `MSBuildProjectExtensionsPath` defeats the environment entirely, and both projects go back to one shared assets file. So `RestoreIsolation` asks *"can this project override the redirection"*, not *"where would its assets file go"*. That is answerable from text, because MSBuild cannot assign a property without naming it literally — if none of the three names appears anywhere reachable, the environment holds. When one might, the scan runs serially. **Found in cross-review.**
  - Every implicitly-imported ancestor file is checked, not just `Directory.Build.props`/`.targets`: `Directory.Packages.props` is pulled in during restore too, and it is the file this tool's users are most likely to have. Leaving it out made a conservative check quietly non-conservative. `Directory.Solution.*` included for the same reason. **Found in cross-review.**
  - Only the standard .NET SDKs are trusted. An SDK-style project implicitly imports its SDK's `Sdk.props`, which no text search can see, so a custom SDK could assign one of the properties unnoticed. `Microsoft.NET.Sdk` assigns `BaseIntermediateOutputPath` *conditionally*, which is exactly why the environment value survives — and every concurrent test in the suite proves it. Anything else is unverified and therefore not trusted. **Also found in cross-review.**
...
### Known limitation, and a pre-existing race worth naming
The opt-in deep scans — `--audit`, `--outdated`, `--deprecated` — also shell out to `dotnet package list`, which also restores. So they have carried the same shared-assets-file race since 3.15.0 made them concurrent, independently of anything here. This release does not fix that; it does mean a repository that configures its own intermediate output now runs its deep scans serially too, since the concurrency decision is shared. That is slower than before and safer than before, and it is the direction to err in. Isolating the deep-scan queries — which would restore full concurrency *and* close the older race — is a separate, separately-reviewable change.
