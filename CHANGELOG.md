...
### Testing
- `PropsOrganizationTests` (20 new) covers determinism across four cultures, ordinal placement of `.` and `-`, insertion into ordered and unordered groups, comment preservation and non-reattachment, the group-header case, style matching in both directions, and idempotence — running a merge on its own output must produce no second diff, which is the property a committed generated file needs above all others. 1022 tests pass.
