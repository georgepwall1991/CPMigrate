...
- JSON contract additions (`outputSchemaVersion` 1.1.0, all additive): `summary.packagesHeldBack`, `summary.verificationRuns`, `summary.bisectBudgetExhausted`, and `packageUpdates[].heldBack`. These are populated for any `--update-packages` run, not only bisecting ones — a plain rollback now reports the whole set as held back — and are omitted from operations that never verify, so no existing field changes meaning.
