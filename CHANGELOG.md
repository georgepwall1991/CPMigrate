...
- **A scan that might redirect its output holds a process-wide lock.** Selecting `maxConcurrency = 1` was not enough under `--batch-parallel`: the shared concurrency gate is sized by whichever solution initialises it first, so redirecting solutions could still run alongside others. **Found in cross-review.**
