...
- **`DirectoryPackagesPropsPath` is honoured.** A repository redirecting central management to a file of its own choosing was still judged against the nearest `Directory.Packages.props` — pins the project never receives, and a file it does not read. Where the redirect names a property that cannot be evaluated, no governing file is claimed and the project is left alone rather than measured against the wrong one.
