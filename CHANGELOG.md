...
- **Analysis is about four times faster on a large solution — measured 39s → 9s on 60 projects.** Resolving packages shells out to `dotnet package list` once per project, a separate process that spends most of its half-second waiting on I/O, and the whole reference pass was serial because *part* of it has to be. That pass is now split along the line that actually matters for correctness:
  ...
  - `MSBuildProjectExtensionsPath` takes precedence over `BaseIntermediateOutputPath` where both are set, since it is the one that decides where the assets file goes — taking whichever appeared first in the XML gave two projects with different base paths but a shared extensions path separate locks. **Found in cross-review.**
  - The lookup never throws. It reads the project file *before* the project is scanned, so an unreadable or unauthorised project would have aborted the entire analysis — where the scanners are equipped to report that one project as an incomplete scan and carry on. A regression introduced by this change and **found in cross-review**.
  - The lock keys on **where the assets file will actually be written**, not just the project directory: a project that redirects `BaseIntermediateOutputPath` or `MSBuildProjectExtensionsPath` shares that file with any other project pointed at the same place, however far apart their directories are. **Found in cross-review.** Read with `XDocument` rather than MSBuild, since this runs in the concurrent phase — so a value set in an imported `Directory.Build.props`, or built from MSBuild properties, is not seen and falls back to the project directory. Documented on the type rather than implied, because a lock that looks total and is not is worse than one with a known edge.
  - **Projects sharing a directory are scanned one at a time, process-wide.** `ProjectDirectoryLock` enforces it across scans rather than within one, because `--batch-parallel` hands several solutions to separate scans at once and two of them can reference projects in the same directory — grouping within a single scan would have left that race open. **Found in cross-review.**
