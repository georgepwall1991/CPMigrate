...
- **`--output Sarif` is now rejected for modes that cannot produce findings.** `--update`, `--interactive`, and `--unify-props` are dispatched before per-command validation, so `--update --output Sarif` previously ran a real self-update and emitted no SARIF at all.
