...
### Fixed
- **`--analyze --fix` could exit `0` with a live vulnerability on disk.** Once any fix applied, the run reported success regardless of what remained. A Critical security advisory is never auto-fixable, so `--fix` repairing an unrelated version inconsistency was enough to report the advisory as a pass. The gate now applies to findings no fixer can repair, which is what is actually left in the tree; a fix that failed to apply is still a failure outright.
